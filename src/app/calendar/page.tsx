...
const CalendarPage = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        // 예약 데이터가 있으면 가장 가까운 예약일을 기본 선택
        if (data.length > 0) {
          const nearest = [...data].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())[0];
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const hasAppointment = appointments.some(app => new Date(app.date).toDateString() === new Date(currentYear, currentMonth, day).toDateString());
