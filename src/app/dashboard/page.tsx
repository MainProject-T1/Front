...
const DashboardPage = () => {
  ...
  // 진단 기록 정렬
  const latestResults = [...myResults]
    .sort((a, b) => new Date(safeDate(b.diagnosisDate)).getTime() - new Date(safeDate(a.diagnosisDate)).getTime())
    .slice(0, 3);
  ...
  return (
    <div className="app-container">
      <div className="page-container" style={{ paddingBottom: '100px' }}>
        ...
        {/* 헤더 */}
        <div style={{
          background: 'linear-gradient(135deg, #ffffff 0%, #f8fafc 30%, #dbeafe 70%, #3b82f6 100%)',
                ...
                {latestResults.map(result => (
                  <div key={result.id} className="flex items-center gap-md">
                    ...
                    <span className="badge" style={{
                      background: safeRiskLevel(result.riskLevel) === 'medium' ? '#fef3c7' : safeRiskLevel(result.riskLevel) === 'high' ? '#fee2e2' : '#dcfce7',
                      color: safeRiskLevel(result.riskLevel) === 'medium' ? '#92400e' : safeRiskLevel(result.riskLevel) === 'high' ? '#dc2626' : '#166534'
                    }}>
                      {getRiskLevelText(safeRiskLevel(result.riskLevel))}
                    </span>
                  </div>
                ))}
              </div>
