...
export default function CameraScreen({ onBack, preloadedStream, onDiagnosisComplete, onCameraReady }) {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        // 비디오 메타데이터가 로드되면 카메라 준비 완료 콜백 호출
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (onDiagnosisComplete) {
          onDiagnosisComplete(dummyDiagnosis, capturedImage);
        }
      }, 1500);
    } catch (err) {
