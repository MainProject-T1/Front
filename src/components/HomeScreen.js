...
export default function HomeScreen({ records, setRecords }) {
  ...
  // 진단 완료 시 기록 추가 콜백
  const handleDiagnosisComplete = (newRecord) => {
    console.log('진단 기록 추가:', newRecord);
    if (typeof setRecords === 'function') {
      setRecords(prev => {
        console.log('이전 기록:', prev);
        return Array.isArray(prev) ? [newRecord, ...prev] : [newRecord];
      });
    }
